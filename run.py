    ...
    while True:
        ...
        for det in detections:
            try:
                ...
                # Convert depth to real-world distance in cm using the method from DepthEstimator
                real_depth_cm = depth_estimator.convert_to_real_depth_cm(depth_val)
                
                depth_text = f"{depth_val:.2f}m"
                real_depth_text = f"{real_depth_cm:.1f}cm"

                label = f"{detector.get_class_names()[class_id]} {depth_text} ({real_depth_text})"
